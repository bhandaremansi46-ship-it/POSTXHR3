...
function addnewcard(newobj, response) {
    ...
    // Add newly created post into local array
    let postObj = {
        ...newobj,
        id: response.id
    };
    ...
    snackbar(
        `The New Post id ${response.id} is added successfully!!`,
        'success'
    );
}
...
function onupdate() {
    ...
    let updateId = Number(localStorage.getItem('EDIT_ID'));
